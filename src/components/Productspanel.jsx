...
const ProductsPanel = () => {
  ...
  const visibleProducts = PRODUCTS.filter(p => !cartItems.some(i => i.productId === p.id));

  return (
    <div>
      <div className="section-title">All Products</div>
      <div className="section-sub">All Products that are available to order</div>
      <div className="product-grid">
        {visibleProducts.map(product => {
          const inWish = wishItems.includes(product.id);
          return (
            <div className="p-card" key={product.id} data-testid={`product-${product.id}`}>
        ...
        {visibleProducts.length === 0 && (
          <p className="section-sub">All products have been added to your cart!</p>
        )}
      </div>
    </div>
  );
